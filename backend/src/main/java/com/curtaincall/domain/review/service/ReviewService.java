...
import com.curtaincall.domain.review.dto.CommentResponse;
import com.curtaincall.domain.review.dto.ReviewRequest;
import com.curtaincall.domain.review.dto.ReviewResponse;
...
public class ReviewService {
    ...
    public ReviewResponse createReview(Long userId, Long showId, ReviewRequest request) {
        if (reviewRepository.existsByShowIdAndUserId(showId, userId)) {
            throw BusinessException.badRequest("이미 이 공연에 대한 리뷰를 작성했습니다.");
        }
        ...
        Review review = reviewRepository.save(Review.builder()
                ...
                .hasSpoiler(Boolean.TRUE.equals(request.getHasSpoiler()))
                .build());
    ...
    public ReviewResponse updateReview(Long userId, Long reviewId, ReviewRequest request) {
        ...
        review.update(
                request.getStoryScore(),
                request.getCastScore(),
                request.getDirectionScore(),
                request.getSoundScore(),
                request.getContent(),
    ...
    public CommentResponse createComment(Long userId, Long reviewId, CommentRequest request) {
        ...
        if (request.getParentId() != null) {
            ...
            if (!parent.getReview().getId().equals(review.getId())) {
                throw BusinessException.badRequest("같은 리뷰의 댓글에만 답글을 달 수 있습니다.");
            }

            if (parent.getParent() != null) {
                throw BusinessException.badRequest("대댓글에는 다시 답글을 달 수 없습니다.");
            }
        }
    ...
    private Page<ReviewResponse> enrichReviews(Page<Review> reviews, Long currentUserId) {
        ...
        if (reviewIds.isEmpty()) {
            return reviews.map(review -> ReviewResponse.from(review, false, 0L));
        }
